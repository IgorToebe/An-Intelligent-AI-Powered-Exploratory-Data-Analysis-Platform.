...
def criar_ferramentas_agente(df: pd.DataFrame) -> List[Any]:
    """
...
**Valores Nulos:** {sum(resultado['valores_nulos']['contagem'].values())} total
    ...
    def realizar_clustering(colunas_features: str, algoritmo: str = "kmeans", n_clusters: int = 3) -> str:
        ...
        try:
            ...
            # Função auxiliar para executar com timeout em Windows
            resultado_clustering = [None, None]  # [clusters, model]
            erro_clustering = [None]
            ...
            def executar_clustering():
                try:
                    ...
                    resultado_clustering[0] = clusters
                    resultado_clustering[1] = model
                except Exception as e:
                    erro_clustering[0] = e
            
            # Executar clustering em thread separada para controle de timeout
            thread_clustering = threading.Thread(target=executar_clustering)
            thread_clustering.start()
            
            # Aguardar até 60 segundos
            thread_clustering.join(timeout=60)
            
            if thread_clustering.is_alive():
                # Thread ainda está executando, timeout atingido
                return f"⚠️ Clustering cancelado por timeout (>60 segundos). Dataset muito grande ou algoritmo complexo. Tente reduzir o tamanho dos dados."
            
            if erro_clustering[0]:
                raise erro_clustering[0]
            
            clusters = resultado_clustering[0]
            model = resultado_clustering[1]
